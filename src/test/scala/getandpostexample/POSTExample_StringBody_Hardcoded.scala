...
class POSTExample_StringBody_Hardcoded extends Simulation {
  ...
  val scn = scenario("CreateLink")
    .exec(http("createnewlink").post("/v1/links").body(StringBody("""{"destination":"https://www.qamilestone.com/","slashtag":"myurl1","title":"myurl1title"}""")).asJson) //send post request
